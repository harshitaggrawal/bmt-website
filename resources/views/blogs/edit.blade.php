...
<main class="flex-1 p-6">
    <div class="bg-gray-800 p-5 rounded shadow">
        ...
        <form action="{{ route('blogs.update', $blog->id) }}" method="POST" enctype="multipart/form-data" class="mt-5 space-y-4">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="author" value="{{ $blog->author }}" required class="bg-gray-700 p-2 rounded w-full text-white">
                </div>
                <div>
                    <label class="block font-semibold">Posted Date</label>
                    <input type="date" name="date_of_post" value="{{ $blog->date_of_post }}" required class="bg-gray-700 p-2 rounded w-full text-white">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="w-full">
                    ...
                    <div class="w-full h-32 bg-gray-700 flex items-center justify-center rounded cursor-pointer" onclick="document.getElementById('outer_image').click()">
                        <img src="{{ asset('storage/' . $blog->outer_image) }}" id="outer_image_preview" class="w-full h-full object-cover rounded ">
                ...
                <div class="w-full">
                    ...
                    <div class="w-full h-32 bg-gray-700 flex items-center justify-center rounded cursor-pointer" onclick="document.getElementById('inner_image').click()">
                        <img src="{{ asset('storage/' . $blog->inner_image) }}" id="inner_image_preview" class="w-full h-full object-cover rounded ">
                        <span class="text-gray-400">Click to Upload</span>
                    </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" value="{{ $blog->title }}" required class="bg-gray-700 p-2 rounded w-full text-white">
            </div>

            <div class="mb-4">
                <label class="block font-semibold">Post URL (ex: /url)</label>
                <input type="text" name="post_url" value="{{ $blog->post_url }}" required class="bg-gray-700 p-2 rounded w-full text-white">
            </div>

            <div class="mb-4">
                <label class="block font-semibold">Meta Title</label>
                <input type="text" name="meta_title" value="{{ $blog->meta_title }}" class="bg-gray-700 p-2 rounded w-full text-white">
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <textarea name="meta_description" class="bg-gray-700 p-2 rounded w-full text-white">{{ $blog->meta_description }}</textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="short_description" required class="bg-gray-700 p-2 rounded w-full text-white">{{ $blog->short_description }}</textarea>
            </div>

            <div class="mb-4">
                <label class="block font-semibold">Long Description</label>
                <textarea name="long_description" id="editor" required class="bg-gray-700 p-2 rounded w-full text-white">{{ $blog->long_description }}</textarea>
            </div>

            <button type="submit" class="bg-green-500 text-white px-4 py-2 rounded">Update Blog</button>
        </form>
    </div>
</main>
